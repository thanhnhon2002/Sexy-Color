...
namespace BBG.PictureColoring
{
    public class PictureArea : MonoBehaviour
    {
        ...
        [Header("Picture Settings")] [SerializeField]
        private PictureScrollArea pictureScrollArea = null;
        ...
        [Header("Number Text Settings")] [SerializeField]
        private Font numberTextFont = null;
        ...
        public PixelClickedHandler OnPixelHold { get; set; }
        ...
        public delegate void PixelClickedHandler(int pixelX, int pixelY, Vector2 screenPosition);

        public delegate void PixelHoldHandler(int pixelX, int pixelY, Vector2 screenPosition);
        ...
        public void Initialize()
        {
            ...
            pictureScrollArea.OnHold += OnPictureAreaHold;
        ...
        public void Setup(int selectedColorIndex)
        {
            ...
            if (PlayerPrefs.GetInt("ColorMaterial", -1) != -1)
            {
                regionMaterial = _materials[PlayerPrefs.GetInt("ColorMaterial")];
            }
            ...
            if (activeLevelData != null)
            {
                ...
                float scale = Mathf.Min(pictureContainer.rect.width / contentWidth,
                    pictureContainer.rect.height / contentHeight, 1f);
        ...
        public void ZoomInOnRegion(Region region)
        {
            ...
            float x1 = (zoomToX - pictureContainer.rect.width / 2f) *
                       (pictureContainer.localScale.x * pictureScrollArea.CurrentZoom);
            float y1 = (zoomToY - pictureContainer.rect.height / 2f) *
                       (pictureContainer.localScale.y * pictureScrollArea.CurrentZoom);
        ...
        private void OnPictureAreaHold(Vector2 screenPosition)
        {
            // Convert the screenPosition to the local position inside pictureContainer
            Vector2 localPosition;

            RectTransformUtility.ScreenPointToLocalPointInRectangle(pictureContainer, screenPosition, canvasCamera,
                out localPosition);
            ...
            if (localPosition.x >= 0 && localPosition.y >= 0 && localPosition.x < pictureContainer.rect.width &&
                localPosition.y < pictureContainer.rect.height)
            {
                OnPixelHold((int)localPosition.x, (int)localPosition.y, screenPosition);
            }
        }
        ...
        private void OnPictureAreaClicked(Vector2 screenPosition)
        {
            ...
            if (localPosition.x >= 0 && localPosition.y >= 0 && localPosition.x < pictureContainer.rect.width &&
                localPosition.y < pictureContainer.rect.height)
            {
                OnPixelClicked((int)localPosition.x, (int)localPosition.y, screenPosition);
            }
        }
        ...
        private void CreatePictureObjects()
        {
            ...
            colorNumbersText = CreateContainerObj("color_numbers_text", pictureContainer)
                .AddComponent<ColorNumbersText>();
