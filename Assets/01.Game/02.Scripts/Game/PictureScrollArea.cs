...
namespace BBG.PictureColoring
{
    public class PictureScrollArea : MonoBehaviour
    {
        ...
        [SerializeField] private float      delayTillHold       = 1;
        ...
        {
            get { return content.transform.localScale.x; }
            set { content.transform.localScale = new Vector3(value, value, 1f); }
        }
        ...
        private void Update()
        {
            ...
            if (delayTillHold != 0 && state == State.Click)
            {
                timeTillHold -= Time.deltaTime;
        ...
        public void ZoomTo(float x, float y, float scale)
        {
            ...
            isAnimating = true;
            fromValues = new Vector3(contentRect.anchoredPosition.x, contentRect.anchoredPosition.y,
        ...
        private void UpdateMouseWheel()
        {
            ...
            {
                // Get the new scale
                float scale = Mathf.Clamp(content.transform.localScale.x + change * mouseWheelZoomScale, minZoom,
                    maxZoom);
                ...
                Vector2 localMiddlePoint;
                RectTransformUtility.ScreenPointToLocalPointInRectangle(contentRectT, middle, null,
                    out localMiddlePoint);
